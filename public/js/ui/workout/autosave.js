...
export function createDraftAutosave({ buildBody }) {
  ...
  const flush = async () => {
    ...
    try {
      ...
      if (wait === undefined) {
        setStatus(
          'error',
          'A napló nincs elmentve — ellenőrizd a kapcsolatot, majd módosíts valamit az újrapróbáláshoz.',
        );
        return;
      }
      retryStep += 1;
      setStatus(
        'error',
        `Nem sikerült menteni — újrapróbálkozás ${Math.round(wait / 1000)} mp múlva…`,
      );
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    if (document.visibilityState !== 'hidden' || (autosaveTimer === null && retryTimer === null))
      return;
    clearTimeout(autosaveTimer);
    clearTimeout(retryTimer);
    autosaveTimer = null;
